...
p_str <- paste0("Prognose vom ",format.Date(prog_d,"%d.%m.%Y"))
...
notizen_str <- paste0(
            "Prognose vom ",format.Date(prog_d,"%d.%m.%Y")," des CoViD-Simulators der Universität des Saarlandes")
...
titel_str <- paste0("Animation: Corona-Neufälle in Hessens Kreisen ",ifelse(d > heute," (Prognose)",""))
...
dw_edit_chart(chart_id = png_id, title = titel_str, intro = 
                paste0("<h1><b style='color:#000000; font-size:xx-large;'>",day(tag),".",month(tag),".</b></h1><br>",
