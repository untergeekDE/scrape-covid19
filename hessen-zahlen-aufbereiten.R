...
# - Lade die Excel-Datei von der Seite www.corona-test-hessen.de
# - Mach es ein bisschen sauberer
# - Schiebe es in die Datawrapper-Grafik
# - Alles weitere passiert dann in Datawrapper. 

teststellen_url <- "https://www.corona-test-hessen.de/data/corona-test-hessen.xlsx?v=6DB82A2FE00C940279CB9D467765690C"

# Das hier ist nur ein kurzer Test - brauchen wir nicht zwingend. Einfach irgendwo abgeschrieben. 
# Wenn man sich das Laden mit ```library(RCurl)``` sparen will, kann man einen Befehl auch direkt
# unter Nennung der Bibliothek verwenden - die Funktion ```url.exists()``` aus der Library RCurl dann so: 

if (RCurl::url.exists(teststellen_url)) {
  msg("Excel-Tabelle auf www.corona-test-hessen.de gefunden")
}
teststellen_roh_df <- read.xlsx(teststellen_url)
t_stellen_df <- teststellen_roh_df %>%  
  # Die tidyverse-"Pipe" %>% zeigt, dass mit den Daten gleich noch was passieren soll
  # Als erstes: Die doppelten Spalten umbenennen - über die Spaltennummer 
  rename(Mo1 = 7,
         Mo2 = 8,
         Di1 = 9,
         Di2 = 10,
         ...
         So1 = 19,
         So2 = 20) %>%
  mutate(Öffnungszeiten = paste0(    # Zeichenketten zu einer zusammenkleben
    "Mo: ",Mo1,Mo2,"<br>",   
    "Di: ",Di1,Di2,"<br>",
    "Mi: ",Mi1,Mi2,"<br>",
    "Do: ",Do1,Do2,"<br>",
    ...
    "Sa: ",Sa1,Sa2,"<br>",
    "So: ",So1,So2,"<br>")) %>%
  # Jetzt die NA wieder rausschmeißen - nicht sehr elegant, aber macht den Job.
  mutate(Öffnungszeiten = str_replace_all(Öffnungszeiten,"NA","")) %>%
  # Jetzt die Spalten auswählen, die wir behalten wollen, 
  # in der Reihenfolge, die wir haben wollen,
  # unter dem Namen, den wir haben wollen: NeuerName=AlterName
  select(Lat=lat,
         Lon=lng,
         Titel=Name,
         Strasse,PLZ,Ort,
  ...
  mutate(Schnelltests = ifelse(Schnelltests!=1 | is.na(Schnelltests),FALSE,TRUE),
         # NA sorgt dafür, dass das IF nicht ausgeführt wird, deshalb müssen wir es abfangen
         PCR = ifelse(PCR!=1 | is.na(PCR),FALSE,TRUE),
         Terminvereinbarung =ifelse(Terminvereinbarung!=1 | is.na(Terminvereinbarung),FALSE,TRUE))

t_stellen_id ="WDHoJ"

msg("Teststellen aktualisieren...")
dw_data_to_chart(t_stellen_df,chart_id=t_stellen_id)
t_stellen_dw <- dw_publish_chart(chart_id=t_stellen_id)
